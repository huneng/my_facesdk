...
void random_samples(SampleSet *set, float **rsdls){
    static cv::RNG rng(cv::getTickCount());
    ...
    for(int i = 0; i < set->ssize; i++){
        ...
        HU_SWAP(set->samples[id1], set->samples[id2], Sample*);
        HU_SWAP(rsdls[id1], rsdls[id2], float*);
    }
}
...
void calculate_residuals(SampleSet *set, int *idxs, int size, int pntIdx, float *rsdls){
    ...
    for(int i = 0; i < size; i++){
        ...
        float dx = sample->oriShape.pts[idx] - sample->curShape.pts[idx];
        float dy = sample->oriShape.pts[idy] - sample->curShape.pts[idy];
