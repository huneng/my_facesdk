...
//#define MINE_CAMERA
    ...
    int app_signature_hash_code = 586517140;//此处为java层调用getSignature得到的值
...
JNIEXPORT jint JNICALL Java_com_visionenergy_mycamera_FaceUtil_loadTracker
    ...
    const char *p1 = env->GetStringUTFChars(modelDir, &copy);
    __android_log_print(ANDROID_LOG_INFO, "jni", "Load model %s\n", p1);


    return load_models(p1);
}
