...
#define MAX_TIME_SIZE 10000
int gTimer = 0;


int load(const char *modelDir, FaceManager **rmanager)
{
    if(time(NULL) > DEAD_TIME)
        return -1;

    FaceManager *manager;

    char detectModel[128], trackModel[128], alignModel[128];

    sprintf(detectModel, "%s/detect_model.dat", modelDir);
    sprintf(trackModel, "%s/track_model.dat", modelDir);
    sprintf(alignModel, "%s/align_model.dat", modelDir);
...
int detect_face(FaceManager *manager, uint8_t *img, int width, int height, int stride, int flag, HRect **resRect){
    ObjectDetector *detector = manager->detector;
    HRect *rects;
    float *confs;
    int rsize;
...
int validate_face(FaceManager *manager, uint8_t *img, int width, int height, int stride, Shape &shape){
    ...
    int ret = predict(manager->detector, patch, WINW, WINH, WINW);
...
int track_mul_face(FaceManager *manager, uint8_t *img, int width, int height, int stride, Shape *resShapes){
    ...
    if(INTER % 25 == 0){
        ...
        rects = NULL;
        rsize = detect_face(manager, img, width, height, stride, 0, &rects);
        if(rsize == 0 && manager->lastSize == 0) return 0;

        if(rects != NULL)
            delete [] rects;

        for(int i = 0; i < manager->lastSize; i++)
            resShapes[i] = manager->lastShapes[i];
    ...
    for(int i = 0; i < manager->lastSize; i++){
        ...
        if(INTER % 5 == 3 && validate_face(manager, img, width, height, stride, shape) == 0){
            INTER = 0;
            flags[i] = 1;
            continue;
        }
...
uint8_t *gImgBuffer = NULL;
uint8_t *gRotateBuf = NULL;
int gCapacity = 0, gCapRotate = 0;


int load_models(const char *modelDir){
    if(time(NULL) > DEAD_TIME)
        return -1;
...
int process_track(uint8_t *img, int width, int height, int stride, int format, int rflag, float *resPts){
    ...
    if(MAX_FACE_SIZE == 1)
        rsize = track_one_face(gManager, imgData, cols, rows, step, shapes[0]);
    else
        rsize = track_mul_face(gManager, imgData, cols, rows, step, shapes);
...
int process_align(uint8_t *img, int width, int height, int stride, int format, float **resShape){
    ...
    if(format == FT_IMAGE_NV21){
        ...
        for(int y = 0; y < height; y++)
            memcpy(gray + y * gStride, buffer + y * stride2, sizeof(uint8_t) * width);
