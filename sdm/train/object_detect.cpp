...
#define HU_VERSION_EPCH 1
#define HU_VERSION_MAJOR 0
#define HU_VERSION_MINOR 0
...
static void integral_image(uint8_t *img, int width, int height, int stride, uint32_t *iImg, int istride){
...
#define HU_FIX_POINT_Q 14

static void resize_gray_image(uint8_t *src, int srcw, int srch, int srcs, uint8_t *dst, int dstw, int dsth, int dsts){
    uint16_t *xtable = NULL;

    uint16_t FIX_0_5 = 1 << (HU_FIX_POINT_Q - 1);
    float scalex, scaley;
    ...
    for(int x = 0; x < dstw; x++){
        ...
        xtable[x * 2] = x0;
        xtable[x * 2 + 1] = (1 << HU_FIX_POINT_Q) - (xc - x0) * (1 << HU_FIX_POINT_Q);
    }
    ...
    for(int y = 0; y < dsth; y++){
        ...
        wy0 = (1 << HU_FIX_POINT_Q) - uint16_t((yc - y0) * (1 << HU_FIX_POINT_Q));
        ...
        for(x = 0; x <= dstw - 4; x += 4){
            ...
            vy0 = (wx0 * (ptrSrc0[0] - ptrSrc0[1]) + (ptrSrc0[1] << HU_FIX_POINT_Q) + FIX_0_5) >> HU_FIX_POINT_Q;
            vy1 = (wx0 * (ptrSrc1[0] - ptrSrc1[1]) + (ptrSrc1[1] << HU_FIX_POINT_Q) + FIX_0_5) >> HU_FIX_POINT_Q;
			ptrDst[0] = (wy0 * (vy0 - vy1) + (vy1 << HU_FIX_POINT_Q) + FIX_0_5) >> HU_FIX_POINT_Q;
			...
			vy0 = (wx0 * (ptrSrc0[0] - ptrSrc0[1]) + (ptrSrc0[1] << HU_FIX_POINT_Q) + FIX_0_5) >> HU_FIX_POINT_Q;
			vy1 = (wx0 * (ptrSrc1[0] - ptrSrc1[1]) + (ptrSrc1[1] << HU_FIX_POINT_Q) + FIX_0_5) >> HU_FIX_POINT_Q;
			ptrDst[1] = (wy0 * (vy0 - vy1) + (vy1 << HU_FIX_POINT_Q) + FIX_0_5) >> HU_FIX_POINT_Q;
			...
			vy0 = (wx0 * (ptrSrc0[0] - ptrSrc0[1]) + (ptrSrc0[1] << HU_FIX_POINT_Q) + FIX_0_5) >> HU_FIX_POINT_Q;
			vy1 = (wx0 * (ptrSrc1[0] - ptrSrc1[1]) + (ptrSrc1[1] << HU_FIX_POINT_Q) + FIX_0_5) >> HU_FIX_POINT_Q;
			ptrDst[2] = (wy0 * (vy0 - vy1) + (vy1 << HU_FIX_POINT_Q) + FIX_0_5) >> HU_FIX_POINT_Q;
			...
			vy0 = (wx0 * (ptrSrc0[0] - ptrSrc0[1]) + (ptrSrc0[1] << HU_FIX_POINT_Q) + FIX_0_5) >> HU_FIX_POINT_Q;
			vy1 = (wx0 * (ptrSrc1[0] - ptrSrc1[1]) + (ptrSrc1[1] << HU_FIX_POINT_Q) + FIX_0_5) >> HU_FIX_POINT_Q;
			ptrDst[3] = (wy0 * (vy0 - vy1) + (vy1 << HU_FIX_POINT_Q) + FIX_0_5) >> HU_FIX_POINT_Q;
        ...
        for(; x < dstw; x++){
            ...
            dst[y * dsts + x] = (wy0 * (vy0 - vy1) + (vy1 << HU_FIX_POINT_Q) + FIX_0_5) >> HU_FIX_POINT_Q;
...
int predict(ObjectDetector *cc, uint8_t *img, int width, int height, int stride)
{
    ...
    integral_image(img, width, height, stride, iImg, cc->WINW + 1);
...
int detect_one_scale(ObjectDetector *cc, float scale, uint32_t *iImg, int width, int height, int stride, HRect *resRect, float *resScores){
    int WINW = cc->WINW;
    int WINH = cc->WINH;
...
int load(ObjectDetector **rcc, const char *filePath){
    ...
    ObjectDetector *cc = new ObjectDetector;
    memset(cc, 0, sizeof(ObjectDetector));
